# change multipliers for different tokens
# start from 2024-07-03 00:00:00, first report at 2024-07-04 00:00:00
...
TOKEN_BY_SEASON = {
    ...
    2: [979_020, 61, dt.date(2024, 9, 1), dt.date(2024, 10, 31)],
    3: [652_680, 61, dt.date(2024, 11, 1), dt.date(2024, 12, 31)],
    4: [326_340, 59, dt.date(2025, 1, 1), dt.date(2025, 2, 28)],
}
    ...
    while from_blk < to_blk:
        ...
        while True:
            try:
                ...
                print('fail to get events, wait for 10 seconds')
                time.sleep(10*i)
                i+=1
                ...
                to_blk_limit = from_blk + max(1, (to_blk_limit - from_blk) >> 1)
...
def _get_ix(row):
    return (np.uint(row.block) << 20) + (np.uint(row.transactionIndex) << 10) + np.uint(row.logIndex)    
...
def get_daily_snapshot_for_ve(ve_contract, snap_ts, df_price, candidate_set=None):
    from_blk = 1
    to_blk = W3.eth.block_number
    ...
    while from_blk < to_blk:
        ...
        while True:
            try:
                events = ve_contract.events.Transfer.create_filter(fromBlock=from_blk, toBlock=to_blk_limit).get_all_entries()
                break
            except ValueError as e:
                m = re.search(r'range should work: \[(0x[0-9a-f]*), (0x[0-9a-f]*)\]', str(e))
                to_blk_limit = int(m.group(2), 0) if m is not None else (to_blk_limit - from_blk) // 2 + from_blk
        for event in events:
            if event['args']['from'] not in BLACKLIST:
                candidate_set.add(event['args']['from'])
            if event['args']['to'] not in BLACKLIST:
                candidate_set.add(event['args']['to'])
        from_blk = to_blk_limit + 1
    wtd_ve_value = {}
    ...
    for addr in list(candidate_set):
...
def get_daily_snapshot_for_reeth(candidate_addresses, from_blk, to_blk, snap_ts, df_eth_price):
    # from_blk = 1
    # to_blk = 123612
    for blk in range(from_blk, to_blk):
        res = W3.eth.get_block(blk, full_transactions=True)
        for tx in res.transactions:
            if tx.value > 0:
                if tx['from'] not in BLACKLIST:
                    candidate_addresses.add(tx['from'])
                if tx['to'] not in BLACKLIST:
                    candidate_addresses.add(tx['to'])
    # print(len(candidate_addresses))
    wtd_reeth_value = {}
    blk_before_snap, _, _, _, _ = _get_block_num_from_ts(snap_ts, W3)
    ...
    # candidate_list = list(candidate_addresses)
    for addr in list(candidate_addresses):
        if type(addr) == str and addr != '' and addr != NULL_ADDR:
            wtd_reeth_value[addr] = W3.eth.get_balance(addr, block_identifier=blk_before_snap) * snap_px
    return wtd_reeth_value, candidate_addresses

# def find_nonzero_reeth_account(candidate_list, end_of_day_blk):
#     # by end of day, double check non zero account
#     nonzero_addr = set()
#     for addr in candidate_list:
#         if type(addr) == str and addr != '' and addr != NULL_ADDR:
#             if W3.eth.get_balance(addr, block_identifier=end_of_day_blk) > 0:
#                 nonzero_addr.add(addr)
#     return nonzero_addr
...
def get_daily_ve_new_mint(token_contract, ve_contract, blk_after_from, blk_before_to):
    df_ve = []
    token_to_ve_events = get_events(
        token_contract.events.Transfer,
        from_blk=blk_after_from, to_blk=blk_before_to,
        arg_filter={'to':ve_contract.address},
        extra_args=['transactionIndex', 'logIndex']
    )
    if len(token_to_ve_events) > 0:
        df_to_ve = pd.DataFrame(token_to_ve_events)
        df_to_ve = df_to_ve[df_to_ve['from'] != NULL_ADDR]
        df_to_ve['value'] = df_to_ve['value'].astype(float)
        df_to_ve = df_to_ve.rename(columns={'from':'delegate'})
        df_to_ve = df_to_ve.drop('to', axis=1)
        df_ve.append(df_to_ve)
    else:
        return pd.Series()
    token_from_ve_events = get_events(
        token_contract.events.Transfer,
        from_blk=blk_after_from, to_blk=blk_before_to,
        arg_filter={'from':ve_contract.address},
        extra_args=['transactionIndex', 'logIndex']
    )
    if len(token_from_ve_events) > 0:
        df_from_ve = pd.DataFrame(token_from_ve_events)
        df_from_ve = df_from_ve[df_from_ve['to'] != NULL_ADDR]
        df_from_ve['value'] = -df_from_ve['value'].astype(float)
        df_from_ve = df_from_ve.rename(columns={'to':'delegate'})
        df_from_ve = df_from_ve.drop('from', axis=1)
        df_ve.append(df_from_ve)
    
    dvc_events = get_events(
        ve_contract.events.DelegateVotesChanged,
        from_blk=blk_after_from, to_blk=blk_before_to,
        extra_args=['transactionIndex', 'logIndex']
    )
    if len(dvc_events) > 0:
        df_dvc = pd.DataFrame(dvc_events)
        df_dvc['ix'] = df_dvc.apply(_get_ix, axis=1)
        df_dvc['votesChg'] = df_dvc['newVotes'].astype(float) - df_dvc['previousVotes'].astype(float)
        df_dvc = df_dvc.groupby(['block', 'transactionIndex', 'delegate']).sum()['votesChg']
    else:
        return pd.Series()

    if len(df_ve) > 0:
        df_ve = pd.concat(df_ve)
        df_ve = df_ve.groupby(['block', 'transactionIndex', 'delegate']).sum()['value']
        df_ve = df_ve[df_ve > 0]


    df_add = pd.merge(df_ve, df_dvc, left_index=True, right_index=True).reset_index()
    return df_add.groupby('delegate').votesChg.sum()
...
def day_by_day_summary(root_path='./', start_date=None, end_date=None):
    # get_daily_snapshot_for_reeth(set(), 1, 123612, None, None)
    # raise
    ...
    reeth_holder_path = root_path+f'daily_data/reeth_holder'
    if not os.path.exists(reeth_holder_path):
        os.makedirs(reeth_holder_path)
    reeth_holder = load_from_pickle(reeth_holder_path+f'/{start_date}.pkl')    
    ...
    for dt_date in pd.date_range(start_date, end_date):
        ...
        snap_ts = date_snap_ts_dict[date_str]

        print('checking new mint ve')
        pearl_voting_power = get_daily_ve_new_mint(contracts['pearl'], contracts['vepearl'], blk_after_from, blk_before_to)
        latest_pearl_px = df_token_price_by_blk[contracts['pearl'].address].iloc[-1].px
        df_summary = _add_column(pearl_voting_power * latest_pearl_px, df_summary, f'new_mint_vepearl')
        rwa_voting_power = get_daily_ve_new_mint(contracts['rwa'], contracts['verwa'], blk_after_from, blk_before_to)
        latest_rwa_px = df_token_price_by_blk[contracts['rwa'].address].iloc[-1].px
        df_summary = _add_column(rwa_voting_power * latest_rwa_px, df_summary, f'new_mint_verwa')
        ...
        if len(dfs_sender_to_fee) > 0:
            ...
            _swp_pcnt = min(SWP_PCNT_CAP, df_swap_points.sum() / daily_rwa_token / df_token_price_by_blk[contracts['rwa'].address].px.iloc[-1])
        ...
        for token_symbol in ['rwa', 'pearl', 'usdc', 'usdr']:
            print(f'checking {token_symbol} balances', flush=True)
            if token_symbol not in tokens_balances:
                print(f'new token included: {token_symbol}')
                tokens_balances[token_symbol] = getTokenBalances(contracts[token_symbol], blk_after_from)
                # print(tokens_balances[token_symbol])
            twab_token, _ = averageTokenInWallet(
                contracts[token_symbol], from_ts, to_ts, blk_after_from, blk_before_to, blk_ts_after_from, blk_ts_before_to,
                df_token_price_by_blk[contracts[token_symbol].address],
                running_balances=tokens_balances[token_symbol]
            )
            df_summary = _add_column(twab_token, df_summary, token_symbol)
        ...
        if len(df_summary) > 0:
            ...
            # df_summary = _add_column(vepearl_wtd_val, df_summary, 'vepearl')        
                    # print(len(reeth_holder))
            reeth_holder = reeth_holder.union(set(df_summary.index))
            reeth_snapshot, reeth_holder = get_daily_snapshot_for_reeth(reeth_holder, blk_after_from, blk_before_to, snap_ts, df_token_price_by_blk[contracts['wreeth'].address])
            df_summary = _add_column(reeth_snapshot, df_summary, 'reeth')
            # print(reeth_snapshot)
            # reeth_holder = find_nonzero_reeth_account(new_holder_list, blk_before_to)
            # print(len(reeth_holder))
            pickle.dump(reeth_holder, open(reeth_holder_path+f'/{next_date_str}.pkl', 'wb'))
            print('reeth snapshot done')

            # reeth_holder = reeth_holder.union(set(df_summary.index))
            vepearl_snapshot = get_daily_snapshot_for_ve(contracts['vepearl'], snap_ts, df_token_price_by_blk[contracts['pearl'].address])
            df_summary = _add_column(vepearl_snapshot, df_summary, 'vepearl')
            verwa_snapshot = get_daily_snapshot_for_ve(contracts['verwa'], snap_ts, df_token_price_by_blk[contracts['rwa'].address])
            df_summary = _add_column(verwa_snapshot, df_summary, 'verwa')
            print('ve snapshot done')    
            ...
            usdc_val = df_summary[[cname for cname in df_summary.columns if cname[:4] == 'usdc']].sum(axis=1)
            df_summary['tvl_pts'] += usdc_val
            df_summary['raw_tvl'] += usdc_val
            usdr_val = df_summary[[cname for cname in df_summary.columns if cname[:4] == 'usdr']].sum(axis=1)
            df_summary['tvl_pts'] += usdr_val
            df_summary['raw_tvl'] += usdr_val
            ...
            df_summary['tvl_pts'] += df_summary['smore'] * 5
            ...
            df_summary['tvl_pts'] += df_summary[[cname for cname in df_summary.columns if cname[-3:] == 'alm']].sum(axis=1) * (7 - 1)
            df_summary['tvl_pts'] += df_summary['arcusd_in_alm'] * (4 - 1) * (7 - 1)
            df_summary['tvl_pts'] += df_summary['ukre_in_alm'] * (5 - 1) * (7 - 1)
            df_summary['tvl_pts'] += df_summary['more_in_alm'] * (2 - 1) * (7 - 1)
            # df_summary['tvl_pts'] += df_summary[[cname for cname in df_summary.columns if cname[-3:] == 'nft']].sum(axis=1) 
            ...
            df_summary['daily_norm_new_ve'] = df_summary['new_mint_verwa'] + df_summary['new_mint_vepearl']
            df_summary['swap_pcnt'] = _swp_pcnt
            if df_summary['daily_norm_new_ve'].sum() == 0:
                print('no new minter!!')
                _tvl_pcnt = 1 - _swp_pcnt
            else:
                df_summary['daily_norm_new_ve'] = df_summary['daily_norm_new_ve'] / df_summary['daily_norm_new_ve'].sum()
                _tvl_pcnt = 1 - _swp_pcnt - NEW_VE_PCNT
            df_summary['daily_norm_pts'] = _tvl_pcnt * df_summary['daily_norm_tvl'] + _swp_pcnt * df_summary['daily_norm_swp'] + NEW_VE_PCNT * df_summary['daily_norm_new_ve']
            ...
            df_summary.to_csv(report_path+f'/daily_summary_v4_{date_str}.csv')
            ...
            for collat_addr, [twab_collat, twab_brw] in twab_by_vault.items():
                twab_by_pool[collat_to_vault[collat_addr]] = [
                    twab_collat * ((LEVERAGE_MULTIPLIER[collat_addr]) if collat_addr in LEVERAGE_MULTIPLIER else 1) + twab_brw * LEVERAGE_MULTIPLIER[contracts['more'].address],
                    twab_collat
                ]
            for pool_addr, twab_by_t in twab_in_alms_by_pool.items():
                twab_by_pool[pool_addr] = [0, 0]
                for _t in twab_by_t:
                    twab_by_pool[pool_addr][0] += twab_by_t[_t] * (LEVERAGE_MULTIPLIER[_t] if _t in LEVERAGE_MULTIPLIER else 1) * 7
                    twab_by_pool[pool_addr][1] += twab_by_t[_t]
                
            total_new_mint_val = daily_rwa_token * 0.05 * 0.9 * df_token_price_by_blk[contracts['rwa'].address].px.iloc[-1] * 365
            total_new_mint_val / (df_summary['new_mint_verwa'].sum() + df_summary['new_mint_vepearl'].sum())
            ...
            apy_by_pool.to_csv(report_path+f'/apy_by_pool_v4_{date_str}.csv')
